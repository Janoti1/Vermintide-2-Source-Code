		...
		new_hooks = {
			{
				...
				func = function (vmf_mod, mod_name, hooked_function, self, mood_name, ...)
					...
					if not (mood_name ~= "skill_huntsman_surge" and mood_name ~= "skill_huntsman_stealth") and vmf_mod:get(vmf_mod.SETTING_NAMES.HUNTSMAN_VISUAL) or not (mood_name ~= "wounded" and mood_name ~= "bleeding_out") and vmf_mod:get(vmf_mod.SETTING_NAMES.WOUNDED) or mood_name == "knocked_down" and vmf_mod:get(vmf_mod.SETTING_NAMES.KNOCKED_DOWN) or mood_name == "heal_medkit" and vmf_mod:get(vmf_mod.SETTING_NAMES.HEALING) then
						return
					end
